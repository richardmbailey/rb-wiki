...
- Checks supplied to external-session `finish` are cooperating-agent attestations. The controller records and gates on them but reruns only its own scope, provenance, payload, approval, budget, and Git checks; operators must not report a deterministic check as passed without running it.
